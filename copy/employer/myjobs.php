...
<!-- Content Wrapper -->
<div class="content-wrapper flex-1 flex justify-center p-6">
    <div class="w-full max-w-4xl">
        <h2 class="text-2xl font-bold text-indigo-900 mb-6">My Jobs</h2>
        <!-- Job List Table -->
        <div class="overflow-auto">
            <table class="table table-hover shadow-md bg-white rounded-lg">
                ...
                <tbody>
                    <?php
                    // Fetching jobs posted by the logged-in employer
                    $id_employer = $_SESSION['id_employer']; // Employer's ID from session
                    ...
                    $sql = "
                        SELECT 
                            ...
                            j.job_title, 
                            j.status, 
                            COUNT(a.id_applicant) as applicants_count, 
                            IF(e.company_name = 'None', CONCAT(e.firstname, ' ', e.lastname), e.company_name) AS employer_name
                        FROM jobs j
                        LEFT JOIN applications a ON j.id_jobs = a.id_jobs
                    ...
                    ?>
                        <tr>
                            <td><?= $row['job_title']; ?></td>
                            <td><?= $row['status']; ?></td>
                            <td><?= $row['applicants_count']; ?></td>
                            <td>
                                <a href="../viewjob.php?id=<?= $row['id_jobs']; ?>" class="btn btn-primary">View</a>
                                <!-- Button to trigger Close Job modal -->
                                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#closeJobModal-<?= $row['id_jobs']; ?>">
                                    Close Job
                                </button>
                            </td>
                        </tr>

                        <!-- Modal for Confirming Close Job -->
                        <div class="modal fade" id="closeJobModal-<?= $row['id_jobs']; ?>" tabindex="-1" role="dialog" aria-labelledby="closeJobModalLabel-<?= $row['id_jobs']; ?>" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="closeJobModalLabel-<?= $row['id_jobs']; ?>">Confirm Close Job</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        Are you sure you want to close this job posting? This will stop any new applications from being submitted.
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                        <a href="be/closejob.php?id=<?= $row['id_jobs']; ?>" class="btn btn-danger">Yes, Close Job</a>
                                    </div>
                                </div>
                            </div>
                        </div>
